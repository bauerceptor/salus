...
if (!window.Expo) { window.Expo = { easeOut: null }; }
if (!window.Quad) { window.Quad = { easeOut: null }; }
if (!window.Power2) { window.Power2 = { easeInOut: null }; }
...
var activeElement, curEmailIndex, screenCenter, svgCoords, emailCoords, emailScrollMax, chinMin = .5, dFromC, mouthStatus = "small", blinking, eyeScale = 1, eyesCovered = false, showPasswordClicked = false, isPasswordVisible = false;
...
function onEmailInput(e) {
    ...
    if(value.length > 0) {
        email.parentElement.classList.add("hasText");
    } else {
        email.parentElement.classList.remove("hasText");
    }
...
function onPasswordInput(e) {
    if(e.target.value.length > 0) {
        e.target.parentElement.classList.add("hasText");
    } else {
        e.target.parentElement.classList.remove("hasText");
    }
}
...
function onPasswordToggleClick(e) {
    ...
    showPasswordToggle.classList.toggle("is-active", isPasswordVisible);
    showPasswordToggle.setAttribute("aria-pressed", isPasswordVisible ? "true" : "false");
    showPasswordToggle.setAttribute("aria-label", isPasswordVisible ? "Hide password" : "Show password");
...
function startBlinking(minDelay, maxDelay) {
    if(minDelay != null && maxDelay != null) {
        delay = getRandomRange(minDelay, maxDelay);
    } else if(minDelay != null) {
        delay = getRandomInt(minDelay);
    } else {
        delay = 1;
    }
...
function initLoginForm() {
    ...
    if(password.value.length > 0) {
        password.parentElement.classList.add("hasText");
    }
    ...
    showPasswordToggle.addEventListener('click', onPasswordToggleClick);
    showPasswordToggle.addEventListener('focus', onPasswordToggleFocus);
    ...
    if(isMobileDevice()) {
        ...
        showPasswordToggle.classList.add("is-active");
        showPasswordToggle.setAttribute("aria-pressed", "true");
        showPasswordToggle.setAttribute("aria-label", "Hide password");
