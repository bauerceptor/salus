...
LLAMA_CPP_BINARY = BASE_DIR / "llama.cpp" / "build" / "bin" / "llama-cli"
LLAMA_CPP_VLM_BINARY = BASE_DIR / "llama.cpp" / "build" / "bin" / "llama-mtmd-cli"
...
class LLMClient:
    ...
    def _check(self):
        ...
        if not self.binary_path.exists():
            # Try common fallback names in build/bin
            fallbacks = [
                self.binary_path.parent / "llama-cli",
                self.binary_path.parent / "main",
            ...
            if not found:
                ...
                print("Searched for: llama-cli, main in llama.cpp/build/bin/")
                sys.exit(1)
...
class VLMClient:
    ...
    def _resolve_binary(self) -> Path:
        candidates = [
            self.binary_path,
            self.binary_path.parent / "llama-mtmd-cli",
            self.binary_path.parent / "llava-cli",
            self.binary_path.parent / "minicpmv-cli",
            self.binary_path.parent / "llama-qwen2vl-cli",
            self.binary_path.parent / "llama-cli",
            BASE_DIR / "llama.cpp" / "build" / "bin" / "llama-mtmd-cli",
            BASE_DIR / "llama.cpp" / "build" / "bin" / "llava-cli",
            BASE_DIR / "llama.cpp" / "build" / "bin" / "llama-cli",
        ]
        for c in candidates:
            if c.exists():
...
def main():
    ...
    p_ask.add_argument("--no-rag", action="store_true", help="Skip document retrieval; use LLM directly")
    ...
    p_voice.add_argument("--no-rag", action="store_true", help="Skip document retrieval; use LLM directly")
