class SpecialistPatientReportService
  ...
  def add_header(pdf)
    ...
    pdf.move_down 10
    pdf.text "Specialist Patient Report", size: 14, style: :italic
  ...
  def add_ai_prediction_section(pdf)
    pdf.text "AI RISK PREDICTION", size: 12, style: :bold
    pdf.move_down 10

    prediction = AdherencePredictionService.new(account).predict_non_adherence_risk(7)

    if prediction.is_a?(String)
      pdf.text prediction, size: 10, style: :italic
    else
      info = [
        ["Risk Level:", prediction[:risk_level]],
        ["Predicted Adherence Rate:", "#{prediction[:predicted_adherence_rate]}%"]
      ]

      table_data = info.map { |label, value| [label, value] }
      pdf.table(table_data, width: 350) do
        columns(0).font_style = :bold
      end

      if prediction[:risk_factors].any?
        pdf.move_down 10
        pdf.text "Risk Factors:", size: 10, style: :bold
        prediction[:risk_factors].each do |factor|
          pdf.text "  • #{factor}", size: 10
        end
      end

      if prediction[:recommendation]
        pdf.move_down 10
        pdf.text "AI Recommendation:", size: 10, style: :bold
        pdf.text prediction[:recommendation], size: 10, style: :italic
      end
    end
  ...
  def add_specialist_notes_section(pdf)
    ...
    notes = specialist.specialist_notes.for_patient(account).order(created_at: :desc).limit(10)
