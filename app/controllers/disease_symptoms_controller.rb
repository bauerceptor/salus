class DiseaseSymptomsController < BaseController
  ...
  def create
    ...
    respond_to do |format|
      if @disease_symptom.save
        format.html do
          redirect_to disease_symptom_path(disease_id: @disease.id, id: @disease_symptom.id, locale: I18n.locale),
                      notice: t(".success")
        end
      else
  ...
  def disease_symptom_params
    params.expect(disease_symptom: %i[name description first_noticed_at disease_id
  ...
  def set_breadcrumbs
    ...
    add_breadcrumb t(".breadcrumbs.index"), disease_symptoms_path(disease_id: @disease.id, locale: I18n.locale)

    case action_name.to_sym
    when :new, :create
      add_breadcrumb t(".breadcrumbs.new"), new_disease_symptom_path(disease_id: @disease.id, locale: I18n.locale)
    when :show
      add_breadcrumb @symptom_name,
                     disease_symptom_path(disease_id: @disease.id, id: @disease_symptom.id, locale: I18n.locale)
    when :edit, :update
      add_breadcrumb @symptom_name,
                     disease_symptom_path(disease_id: @disease.id, id: @disease_symptom.id, locale: I18n.locale)
      add_breadcrumb t(".breadcrumbs.edit"),
                     edit_disease_symptom_path(disease_id: @disease.id, id: @disease_symptom.id, locale: I18n.locale)
    end
  end
end
