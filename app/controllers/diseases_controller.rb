class DiseasesController < BaseController
  ...
  def update
    ...
    respond_to do |format|
      if @disease.update(disease_params)
        format.html { redirect_to disease_url(id: @disease.id, locale: I18n.locale), notice: t(".success") }
      else
  ...
  def disease_params
    params.expect(disease: %i[name diagnosed_at diagnosed_by_hp severity color
  ...
  def set_breadcrumbs
    ...
    when :edit, :update
      add_breadcrumb @disease.predefined_disease.name, @disease
      add_breadcrumb t(".breadcrumbs.edit"), edit_disease_path(id: @disease.id, locale: I18n.locale)
    end
  end
end
