class TreatmentUpdatesController < BaseController
  ...
  def treatment_update_params
    params.expect(treatment_update: %i[name status description update_date
                                       treatment_id])
  end
  ...
  def set_breadcrumbs
    ...
    add_breadcrumb @treatment.title, treatment_path(id: @treatment.id, locale: I18n.locale)

    case action_name.to_sym
    when :new, :create
      add_breadcrumb t(".breadcrumbs.new"),
                     new_treatment_treatment_update_path(treatment_id: @treatment.id, locale: I18n.locale)
    when :edit, :update
      add_breadcrumb t(".breadcrumbs.edit"),
                     edit_treatment_treatment_update_path(treatment_id: @treatment.id, id: @treatment_update.id,
                                                          locale: I18n.locale)
    end
  end
end
