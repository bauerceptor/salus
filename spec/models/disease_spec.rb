...
RSpec.describe Disease, type: :model do
  ...
  describe "callbacks" do
    describe "after_create :check_group_onboarding" do
      it "does not raise when disease is created" do
        account = create(:account)
        predefined_disease = create(:predefined_disease)
        expect do
          create(:disease, account: account, predefined_disease: predefined_disease)
        end.not_to raise_error
      end

      it "runs on create but not on update" do
        account = create(:account)
        predefined_disease = create(:predefined_disease)
        disease = create(:disease, account: account, predefined_disease: predefined_disease)

        expect do
          disease.update!(severity: 3)
        end.not_to raise_error
      end
    end
  end
